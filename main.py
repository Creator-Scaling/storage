...
=================================
Commands:
  discover  — Scrape Instagram/YouTube leads, qualify with Claude, write to Supabase
  enrich    — Find emails for leads in Supabase that are missing one
  research  — Process "Approved" leads: deep research + Claude audit + HTML page
...
def discover(niches, posts_per_hashtag, limit, dry_run):
    ...
    click.echo(f"\n[4/4] Enriching and writing {len(qualified)} leads to Supabase...")
    ...
    click.echo(f"  Written {added} leads to Supabase.")
    click.echo("\nDone. Open Supabase, review leads, and set status to 'Approved' to trigger research.")


@cli.command()
def enrich():
    """Find and fill in missing emails for Pending Review leads."""
    from storage.supabase import get_leads_by_status, update_lead_email, STATUS_PENDING
    from enrichment.apollo import find_email

    click.echo("\nFetching leads missing emails...")
    leads = get_leads_by_status(STATUS_PENDING)
    ...
    for i, lead in enumerate(missing):
        ...
        if email:
            update_lead_email(lead["id"], email, source)
...
def research(limit):
    """Run deep research + generate audit HTML pages for Approved leads."""
    from storage.supabase import (
        get_leads_by_status, update_lead_status,
        STATUS_APPROVED, STATUS_RESEARCHING, STATUS_AUDITED,
    )
    from research.researcher import research_lead
    from audit.generator import generate_audit_page

    approved = get_leads_by_status(STATUS_APPROVED)
    if not approved:
        click.echo("No leads with status 'Approved' found in Supabase.")
        return
    ...
    for i, lead in enumerate(approved):
        name = f"{lead.get('first_name', '')} {lead.get('last_name', '')}".strip() or lead.get("instagram_handle", "?")
        click.echo(f"[{i+1}/{len(approved)}] {name} (@{lead.get('instagram_handle')})")

        update_lead_status(lead["id"], STATUS_RESEARCHING)

        try:
            research_data = research_lead(lead)
            page_url = generate_audit_page(research_data)
            update_lead_status(lead["id"], STATUS_AUDITED, audit_url=page_url)
            click.echo(f"  Audit page: {page_url}\n")
        except Exception as exc:
            click.echo(f"  ERROR: {exc}")
            update_lead_status(lead["id"], STATUS_APPROVED)
...
@cli.command()
def status():
    """Print a summary of lead counts by status."""
    from storage.supabase import get_status_counts
